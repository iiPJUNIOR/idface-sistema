      ...
      {presence.map((p, idx) => (
        <div key={p.id} className="group flex items-center gap-5 p-5 glass-card rounded-2xl hover:-translate-y-1 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-300 animate-slide-in relative overflow-hidden" style={{ animationDelay: `${idx * 40}ms` }}>
          ...
          <div className="flex-1">
            <p className="text-xl font-heading font-semibold text-white tracking-tight">{p.name}</p>
            <p className="text-sm font-medium text-slate-400">Mat: {p.registration}</p>
          </div>
          <div className="text-right">
             <div className="bg-white/5 rounded-xl px-4 py-2 border border-white/5 flex flex-col items-center">
                <p className="text-lg font-bold text-white font-mono tracking-wider drop-shadow-sm">
                  {new Date(p.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                </p>
                {p.entries_count > 1 && (
                  <p className="text-xs font-semibold text-indigo-400 uppercase tracking-wider mt-0.5">{p.entries_count} ACESSOS</p>
                )}
             </div>
          </div>
        </div>
